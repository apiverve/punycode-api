...
class PunycodeAPIClientError(Exception):
...
class ValidationError(PunycodeAPIClientError):
...
class PunycodeAPIClient:
    # Validation rules for parameters (generated from schema)
    VALIDATION_RULES = {"text": {"type": "string", "required": True, "maxLength": 1000}, "mode": {"type": "string", "required": False, "default": "encode"}}
    ...
    def __init__(self, api_key: str, secure: bool = True, debug: bool = False):
        """
        Initialize the PunycodeAPIClient with the API key and optional settings

        :param api_key: Your APIVerve API key
        :param secure: Deprecated. Always set to True
        :param debug: Enable debug logging (default: False)
        :raises PunycodeAPIClientError: If API key is invalid
        """
        ...
        self.secure = secure
        self.debug = debug
        self.base_url = 'https://api.apiverve.com/v1/punycode'
        self.headers = {
    ...
    def _log(self, message: str) -> None:
        ...
        """
        if self.debug:
            print(f"[PunycodeAPIClient DEBUG] {message}")
    ...
    def execute(self, params: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:  # Returns PunycodeConverterData in 'data' field
        """
        Execute the API request

        :param params: API parameters (optional)
        :return: API response as a dictionary
        :raises PunycodeAPIClientError: If the request fails or returns an error
        ...
        try:
            ...
            raise PunycodeAPIClientError(f"Request timeout: {str(e)}")
        except requests.exceptions.ConnectionError as e:
            self._log(f"Connection error: {str(e)}")
            raise PunycodeAPIClientError(f"Connection error: {str(e)}")
        except requests.exceptions.RequestException as e:
            self._log(f"Request exception: {str(e)}")
            raise PunycodeAPIClientError(f"Request failed: {str(e)}")
